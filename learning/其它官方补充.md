...
- 深入了解组件
  ...
  + Prop
    - [Prop类型](#Prop类型)
    - [传递静态或动态Prop](#传递静态或动态Prop)
    - [单向数据流](#单向数据流)
    - [自定义属性验证](#自定义属性验证)
      + [类型检查范围](#类型检查范围)
    - [非Prop的attribute](#非Prop的attribute)
      + [替换/合并已有的attribute](#替换/合并已有的attribute)
      + [禁用attribute继承](#禁用attribute继承)
  + [自定义事件](#自定义事件)
    - [事件名](#事件名)
    - [将原生事件绑定到组件](#将原生事件绑定到组件)
    - [sync修饰符](#sync修饰符)
  + [动态组件&异步组件](#动态组件&异步组件)
    - [在动态组件上使用keep-alive](#在动态组件上使用keep-alive)
    - [异步组件](#异步组件)
    - [处理加载状态](#处理加载状态)
...
```
// 父组件  
<test v-bind:title.sync="doc.title"></test>
```
