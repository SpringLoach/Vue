...
- [登录相关](#登录相关)  
  ...
  + [表单的重置功能](#表单的重置功能)
  + [表单预验证](#表单预验证)
  + [配置axios发起登录请求](#配置axios发起登录请求)
  + [从响应中获取token并保存](#从响应中获取token并保存)
  + [强制跳转到登录页](#强制跳转到登录页)
  + [退出登录](#退出登录)
  + [将token添加到请求中](#将token添加到请求中)
- [菜单](#菜单)
  + [获取菜单数据](#获取菜单数据)
  + [菜单的点击模式](#菜单的点击模式)
  + [菜单的缩放和悬停](#菜单的缩放和悬停)
...
- [用户管理](#用户管理)
  ...
  + [思路-子组件复用](#思路-子组件复用)
  + [思路-多功能子组件](#思路-多功能子组件)
  + [思路-子组件遍历项中绑动态事件和属性](#思路-子组件遍历项中绑动态事件和属性)
  + [思路-间距悬浮](#思路-间距悬浮)
  + [抽离普通组件](#抽离普通组件)
- 用户管理功能
  + [添加用户](#添加用户)
...
- [vue-table-with-tree-grid](#vue-table-with-tree-grid)
- [使用vue-quill-editor文本编辑](#使用vue-quill-editor文本编辑)
- [使用echarts数据可视化](#使用echarts数据可视化)
- 其它
  + [深拷贝](#深拷贝)
  + [单独记录遍历项的状态](#单独记录遍历项的状态)
  + [清空空字符串](#清空空字符串)
  + [全局注册时间格式化过滤器](#全局注册时间格式化过滤器)
  + [增加遍历项](#增加遍历项)
  + [网络数据处理思路](#网络数据处理思路)
  + [对话框处理](#对话框处理)
  + [插件样式的改变](#插件样式的改变)
- [项目优化](#项目优化)
  + [NProgress进度条](#NProgress进度条)
  + [解决ESlinst语法错误](#解决ESlinst语法错误)
  + [打包阶段移除console](#打包阶段移除console)
  + [生成打包报告](#生成打包报告)
  + [修改webpack默认配置](#修改webpack默认配置)
  + [为开发/发布模式指定不同的打包入口](#为开发/发布模式指定不同的打包入口)
  + [通过chainWebpack自定义打包入口](#通过chainWebpack自定义打包入口)
  + [通过externals加载外部CDN资源](#通过externals加载外部CDN资源)
  + [通过CDN优化ElementUI的打包](#通过CDN优化ElementUI的打包)
  + [定制加载方式](#定制加载方式)
  + [路由懒加载分组](#路由懒加载分组)
- [项目上线](#项目上线)
  + [通过node创建web服务器](#通过node创建web服务器)
  + [为Histor模式配置中间件](#为Histor模式配置中间件)
  + [开启gzip配置](#开启gzip配置)
  + [配置https服务](#配置https服务)
  + [使用pm2管理应用](#使用pm2管理应用)
...
#### 使用echarts数据可视化  
> ui界面 -> 安装依赖 -> 运行依赖 -> [使用echarts数据可视化](https://echarts.apache.org/zh/tutorial.html#5%20分钟上手%20ECharts) -> 安装。
> 
...
自定义属性字段  
> 分别配置到[发布模式](通过chainWebpack自定义打包入口)和开发模式的函数中。  
```
// 发布模式
...
![配置https](./img/配置https.jpg)

![配置https2](./img/配置https2.jpg)
