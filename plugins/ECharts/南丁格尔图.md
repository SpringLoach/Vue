- [自定义南丁格尔图](#自定义南丁格尔图)
- [样式简介](#样式简介)
  + [颜色主题（Theme）](#颜色主题（Theme）)
  + [调色盘](#调色盘)
  + [直接的样式设置](#直接的样式设置)
  + [高亮样式emphasis](#高亮样式emphasis)
- [异步数据加载和更新](#异步数据加载和更新)
  + [loading动画](#loading动画)
  + [数据的动态更新](#数据的动态更新)
- [使用dataset管理数据](#使用dataset管理数据)
  + [简单柱状图](#简单柱状图)
  + [数据到图形的映射](#数据到图形的映射)
  + [维度](#维度)
    - 维度名称
    - 单独定义维度
  + [数据到图形的映射(series.encode)](#数据到图形的映射(series.encode))
  + [默认的encode](#默认的encode)
  + [多个dataset的使用](#多个dataset的使用)
  + [系列中的数据](#系列中的数据)
- [使用transform进行数据转换](#使用transform进行数据转换)
  + [数据转换基础使用](#数据转换基础使用)
  + [数据转换的进阶使用](#数据转换的进阶使用)
  + [链式声明transform](#链式声明transform)
  + [在开发环境中debug](#在开发环境中debug)
  + [数据转换器filter](#数据转换器filter)
  + [关于维度](#关于维度)
  + [关于关系比较操作符](#关于关系比较操作符)
  + [关于逻辑比较](#关于逻辑比较)
  + [关于解析器](#关于解析器)
...
- [移动端自适应](#移动端自适应)
  ...
  + [媒体查询](#媒体查询)
- [数据的视觉映射](#数据的视觉映射)
- [ECharts中的事件和行为](#ECharts中的事件和行为)
  + [鼠标事件的处理](#鼠标事件的处理)
  + [组件交互的行为事件](#组件交互的行为事件)
  + [代码触发ECharts中组件的行为](#代码触发ECharts中组件的行为)
  + [监听“空白处”的事件](#监听“空白处”的事件)
- [使用Canvas或者SVG渲染](#使用Canvas或者SVG渲染)
  + [使用渲染器](#使用渲染器)
- [无障碍访问](#无障碍访问)
...
parser | [解析器](#关于解析器) | 'time'、'trim'、'number'
...
#### 使用外部的数据转换器  
> [略](https://echarts.apache.org/zh/tutorial.html#使用%20transform%20进行数据转换)。  
...
*obj* | 可以包含可选的一个或多个[属性](https://echarts.apache.org/zh/tutorial.html#ECharts%20中的事件和行为) 
